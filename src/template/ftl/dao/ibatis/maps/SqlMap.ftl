...
<sqlMap namespace="${table_name}">
	...
	<select id="select${domain_name}List" resultMap="${domain_entity}ResultForList" parameterClass="${domain_entity}" cacheModel="oneDayCache">
		...
		select * from ${table_name} where 1 = 1
		<include refid="sf-${domain_entity}" />
		order by ${first_column?lower_case} desc
		<isNotEmpty property="row.count">
	...
	<select id="select${domain_name}PaginatedList" resultMap="${domain_entity}Result" parameterClass="${domain_entity}" cacheModel="oneDayCache">
	...
	<insert id="insert${domain_name}" parameterClass="${domain_entity}">
		<selectKey resultClass="long" keyProperty="${first_column}">select SEQ_BASE.nextval as ${first_column} from dual</selectKey>
	...
	<update id="update${domain_name}" parameterClass="${domain_entity}">
		...
		where
		<isNotEmpty prepend=" " property="${first_column}">${first_column?lower_case} = #${first_column}#</isNotEmpty>
		<isEmpty prepend=" " property="${first_column}">
			<isNotEmpty prepend=" " property="map.pks">
				${first_column?lower_case} in
