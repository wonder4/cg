...
public class GenerateServiceImplServiceImpl extends BaseFacadeImpl implements GenerateServiceImplService {
	...
	public void generateServiceImpl(TableInfo tableInfo, GenerateCodeConfig generateCodeConfig) {
		...
		String class_path = generateCodeConfig.getBasePath()
				+ StringUtils.substringBeforeLast(ftlPathConfig.getServiceImpl_ftl_path(), "/").concat("/");
		String fileName = class_path.concat(domain_name).concat("ServiceImpl.java");
		File file = new File(fileName);
		String data = generateCodeConfig.getFacade().getTemplateService().getContent(
				ftlPathConfig.getServiceImpl_ftl_path(), model);
		...
		printMsg(tableInfo.getTable_name(), "ServiceImpl");
	}
}
