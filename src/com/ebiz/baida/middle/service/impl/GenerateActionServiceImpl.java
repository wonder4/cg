...
public class GenerateActionServiceImpl extends BaseFacadeImpl implements GenerateActionService {
	...
	public void generateAction(TableInfo tableInfo, GenerateCodeConfig generateCodeConfig) {
		String domain_name = BaseUtils.ChangeClassName(tableInfo.getTable_name());
		String class_name = BaseUtils.ChangeClassName(tableInfo.getTable_name()).concat("Action");
		...
		model.put("int_java_type", DBTypeToJavaTypeUtil.getJavaType("INT", generateCodeConfig.getDb_type()));

		String class_path = generateCodeConfig.getBasePath()
				+ StringUtils.substringBeforeLast(ftlPathConfig.getAction_ftl_path(), "/").concat("/");
		String fileName = class_path.concat(class_name).concat(".java");
		File file = new File(fileName);
		String data = templateService.getContent(ftlPathConfig.getAction_ftl_path(), model);
		...
		printMsg(tableInfo.getTable_name(), "Action");
